...
//                LOG.info("PDF成交方式 : " + inputCSVData.getCustomsDeclarationData().getTransactionMethod());
                ...
                String temStr = lines.get(i - 1).substring(lines.get(i - 1).indexOf("/") + 1);
//                temStr = temStr.replace("CNY/", "");
                inputCSVData.getCustomsDeclarationData().setFreight(new BigDecimal(temStr.substring(0, temStr.indexOf("/"))));
...
//        LOG.info("PDF总价 : " + inputCSVData.getCustomsDeclarationData().getTotalPrice());
        ...
        // 件数不等
        if (!cdmsData.getCountCDMS().equals(inputCSVData.getCustomsDeclarationData().getCount())) {
            inputCSVData.setMisMatch(inputCSVData.getMisMatch() + "件数不等/");
        }
        // 毛重不等
        BigDecimal tempGrossWeightCDMD = cdmsData.getGrossWeightCDMS();

        if (((tempGrossWeightCDMD.compareTo(new BigDecimal(1)) == -1) ? new BigDecimal(1) : tempGrossWeightCDMD).compareTo(inputCSVData.getCustomsDeclarationData().getGrossWeight()) != 0) {
            inputCSVData.setMisMatch(inputCSVData.getMisMatch() + "毛重不等/");
        }
        // 运费不等
        if (cdmsData.getFreightCDMS().compareTo(inputCSVData.getCustomsDeclarationData().getFreight()) != 0) {
            inputCSVData.setMisMatch(inputCSVData.getMisMatch() + "运费不等/");
        }
        // 价值不等
        if (cdmsData.getOriginalValueCDMS().compareTo(inputCSVData.getCustomsDeclarationData().getTotalPrice()) != 0) {
            inputCSVData.setMisMatch(inputCSVData.getMisMatch() + "价值不等/");
        }

        boolean currencyCanRead = false;

        // 币别不一致
        switch (cdmsData.getCurrencyTypeCDMS()) {
            case "CNY":
        ...
        if (inputCSVData.getCustomsDeclarationData().getTransactionMethod().equals("CIF") && inputCSVData.getCustomsDeclarationData().getFreight().compareTo(new BigDecimal(0)) != 0) {
